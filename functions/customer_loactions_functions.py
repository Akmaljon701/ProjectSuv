...
def all_customer_locations(search, page, limit, db):
    if search :
        search_formatted = "%{}%".format(search)
        search_filter = (Customer_locations.name.like(search_formatted))
    else :
        search_filter = Customer_locations.id > 0
    customer_locations = db.query(Customer_locations).filter(search_filter).order_by(Customer_locations.name.asc())
...
def update_customer_loactions_y(form,db,this_user):
    if get_in_db(db, Customer_locations, form.id):
        db.query(Customer_locations).filter(Customer_locations.id == form.id).update({
            ...
            Customer_locations.branch_id: form.branch_id,
