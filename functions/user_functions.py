...
def all_users(search, page, limit, status, db,branch_id):
...
def update_user_r(form, db, thisuser):
    ...
    db.query(Users).filter(Users.id == form.id).update({
        ...
        Users.branch_id: form.branch_id,
    ...
    for i in form.phones:
        ...
        comment = i.comment
        number = i.number
        update_phone(phone_id, comment, number, form.id, thisuser.id, db, 'user',branch_id = form.branch_id)
