...
def all_files_r(search, page, limit, db):
    if search :
        search_formatted = "%{}%".format(search)
        search_filter = (Uploaded.comment.like(search_formatted))
    else :
        search_filter = Uploaded.id > 0
    uploaded = db.query(Uploaded).filter(search_filter).order_by(Uploaded.source.asc())
